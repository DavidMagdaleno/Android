...
class ListadoActivity : AppCompatActivity() {
    ...
    fun leeFicheroJSON(){
        try {
            ...
            //for (i in 0 until DogArray.length()) {//para limitar el numero de datos que salen
    ...
    fun getUsers2() {
        ...
        call.enqueue(object : Callback<Perros> {
            override fun onResponse(call: Call<Perros>, response: Response<Perros>) {
                ...
                for(i in 0..50){
                    message2.add(message[i]) //metodo para reducir el numero de elementos que se muetran
                }
    ...
    fun getBuscarUnUsuario2(idBusc:String){
        ...
        val call = request.getUsuarioss2(idBusc);
        var message2=ArrayList<String>()
        call.enqueue(object : Callback<Perros> {
            override fun onResponse(call: Call<Perros>, response: Response<Perros>) {
                val post = response.body()//se pone con un for si es un arraylist (mirar ejemplo ClienteApiRest)
    ...
    fun getBuscarUnUsuario2(idBusc:String){
        ...
        call.enqueue(object : Callback<Perros> {
            override fun onResponse(call: Call<Perros>, response: Response<Perros>) {
                val post = response.body()//se pone con un for si es un arraylist (mirar ejemplo ClienteApiRest)
                if (post != null) {
                    message.add(post.message.toString())//el add solo se pone si en userapi hay un mutable list
                    status= post.status.toString()//cambiar si es un arraylist
                    //mobileNumbers.add(post.tags.toString())
                }
                if (response.isSuccessful){
