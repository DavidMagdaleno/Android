...
class Conexion:
    ...
    def login(self, dni, clave):
        try:
            self.conectar()
            with self._conexion.cursor() as cursor:
                # En este caso no necesitamos limpiar ningún dato
                cursor.execute("SELECT DNI, Nombre, Clave, Tfno FROM personas WHERE DNI = %s AND Clave = %s", (dni,clave))
                ...
                #print(r)
                if (r):
                    cursor.execute("SELECT roles.id, roles.descripcion FROM personas, roles, rolesasignados WHERE personas.DNI = %s AND personas.DNI = rolesasignados.DNIRol AND roles.id = rolesasignados.idRol ", (dni))
                    r = [dict((cursor.description[i][0], value) for i, value in enumerate(row)) for row in cursor.fetchall()]
